...
if($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    if($pencarian != '') { ?>
      ...
      <?php
        $sql = "SELECT * FROM surat_masuk WHERE no_surat LIKE '%$pencarian%'";
        $query = $sql;
        $queryJml = $sql;
    } else {
        $query = "SELECT * FROM surat_masuk  LIMIT $posisi, $batas";
        $queryJml = "SELECT * FROM surat_masuk ";
    }
} else {
    $query = "SELECT * FROM surat_masuk  LIMIT $posisi, $batas";
    $queryJml = "SELECT * FROM surat_masuk ";
        ...
        <tr>
            ...
            <td><?=substr($data['isi_ringkas'],0,200)?></br><hr></hr><strong>File :</strong>
              ...
              if(!empty($data['file_surat'])){ ?>
                  <strong><a href="?page=sm&action=cek&no=<?=$data['no_surat'];?>"><?=$data['file_surat']?></a></strong>
                  <?php
              } else { ?>
              ...
              <td>
                ...
                if(@$_SESSION['admin']){?>
                    <center><button><a href="#">
                        ...
                        <a href="?page=sm&action=edit&no=<?php echo $data['no_surat'];?>">
                             Edit</a> ||
                        <a href="laporan/cetak_disp.php?no=<?php echo $data['no_surat'];?>" target="_blank">
                             PRINT DISPOSISI</a> ||
                        <a href="?page=sm&action=hapus&no=<?php echo $data['no_surat'];?>" onclick="return confirm('Yakin Akan menghapus data?')">
                             DELETE</a>
                             <?php
                } ?>
                    </td>
